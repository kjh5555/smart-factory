...
async function updateEquipmentStatus() {
  try {
    console.log('센서 모듈 생산 중인 설비 상태 업데이트 시작...');
    
    // 센서 모듈 제품 ID 찾기
    const sensorModule = await prisma.product.findFirst({
      where: {
        name: '센서 모듈'
      }
    });
    ...
    if (!sensorModule) {
      console.log('센서 모듈 제품을 찾을 수 없습니다.');
      return;
    }
    
    // 센서 모듈의 진행 중인 생산 계획 찾기
    const production = await prisma.production.findFirst({
      where: {
        productId: sensorModule.id,
        status: 'IN_PROGRESS'
      }
    });
    
    if (!production) {
      console.log('센서 모듈의 진행 중인 생산 계획을 찾을 수 없습니다.');
      
      // 상태가 PLANNED인 센서 모듈 생산 계획을 IN_PROGRESS로 변경
      ...
      if (!plannedProduction) {
        console.log('센서 모듈의 PLANNED 상태 생산 계획을 찾을 수 없습니다.');
        return;
      }
      ...
      console.log(`센서 모듈 생산 계획 ${plannedProduction.id}의 상태를 IN_PROGRESS로 변경했습니다.`);
